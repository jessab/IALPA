% author: 
% Based on the implementation of Or Sharir, found on
% http://dtai.cs.kuleuven.be/CHR/old/examples/sudoku_sharir.pl
% 
...
cell(_,_,pos([]))==>fail.
cell(Row,_,num(Num))\cell(Row,Col,pos(Pos))<=>swi:select(Num,Pos,NewPos)|cell(Row,Col,pos(NewPos)).
cell(_,Col,num(Num))\cell(Row,Col,pos(Pos))<=>swi:select(Num,Pos,NewPos)|cell(Row,Col,pos(NewPos)).
cell(_Row,_Col,num(Num))\cell(Row,Col,pos(Pos))<=>
	square_number(_Row,_Col,Square),square_number(Row,Col,Square),
	swi:select(Num,Pos,NewPos)|cell(Row,Col,pos(NewPos)).
...
loop:- chr_get_constraint(cell(_,_,pos(_))),guess,loop.
loop:- \+chr_get_constraint(cell(_,_,pos(_))),!.
...
print_board_(P):-
	Row is P//9 + 1,
	Col is P mod 9 + 1,
	chr_get_constraint(cell(Row,Col,num(N))),
	write(N),write(' '),
	(Col == 9 -> nl;true),
	NewP is P+1,
	print_board_(NewP).
